...
static int add_ret(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	list_for_each_entry(lc, &instance_list, node) {
		...
		lc_w("Processing new disk [%s]\n", d->disk->disk_name);
		
		mutex_lock(&lc->disk_lock);
		if (disk->diskseq > lc->diskseq) {
			if (lc->blockdev) {
				mutex_lock(&lc->io_lock)
					// open blockdev
					// script
					if (wait_for_completion_io_timeout(&lc->ios_finished, lc->opts.io_timeout_jiffies)) {
						// putbd
					}
